      ...
      <div className={`flex-1 flex flex-col overflow-hidden ${darkMode ? 'bg-[#0d0f17]' : 'bg-gray-50'}`}>
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className={`lg:col-span-4 ${darkMode ? 'bg-gray-900/50 border-gray-800' : 'bg-white border-gray-200'} backdrop-blur-sm border rounded-2xl p-6 shadow-xl ${darkMode ? 'shadow-gray-900/50' : 'shadow-gray-200/50'} hover:shadow-2xl transition-all duration-300`}>
                  ...
                  {userStats.totalProjects === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>No activity yet. Start by creating your first project!</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      <p className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Recent activities will appear here</p>
                    </div>
                  )}

                  <Link href="/activity" className={`block text-center mt-4 text-sm font-semibold ${darkMode ? 'text-orange-400 hover:text-orange-300' : 'text-orange-600 hover:text-orange-700'} transition-colors`}>
                    View All Activity →
                  </Link>
                </div>
              </div>
          ...
          {userPlan === "professional" && (
            <>
              {/* All Starter Features Plus Pro Features */}
              <div className="mb-6">
                <p className="text-sm text-muted-foreground">Professional Dashboard - Building advanced widgets...</p>
                {/* Will add Pro-specific widgets here */}
              </div>
            </>
          )}
          ...
          {userPlan === "enterprise" && (
            <>
              ...
              <div className="mb-6">
                <p className="text-sm text-muted-foreground">Enterprise Dashboard - Building AI-powered widgets...</p>
                {/* Will add Enterprise-specific widgets here */}
              </div>
            </>
          )}
        </main>
