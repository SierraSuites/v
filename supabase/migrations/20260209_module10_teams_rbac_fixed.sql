-- ============================================
-- MODULE 10: TEAMS & RBAC - DATABASE SCHEMA (FIXED)
-- ============================================
-- Created: February 9, 2026
-- Fixed: February 10, 2026 - Made permission functions IMMUTABLE for RLS
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_custom_roles_company ON custom_roles(company_id) WHERE company_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_custom_roles_slug ON custom_roles(company_id, role_slug);
CREATE INDEX IF NOT EXISTS idx_custom_roles_active ON custom_roles(company_id, is_active) WHERE is_active = true;
...
CREATE TABLE IF NOT EXISTS user_role_assignments (
  ...
  CHECK (project_ids IS NULL OR array_length(project_ids, 1) > 0)
);
...
CREATE INDEX IF NOT EXISTS idx_audit_logs_pending_approval ON audit_logs(company_id, requires_approval, timestamp DESC)
...
-- Function: Check if user has specific permission (IMMUTABLE for RLS)
...
LANGUAGE plpgsql
SECURITY DEFINER
IMMUTABLE  -- Changed from STABLE to IMMUTABLE
AS $$
DECLARE
...
-- Function: Get user's highest role
...
-- Simplified policies without function calls in predicates
